<template>
    <nav class="mt-4">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            <li
                class="inline-flex items-center"
                @click="$emit('selected-breadcrumb', 'Home')"
            >
                <router-link
                    to="/"
                    class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white"
                >
                    <svg aria-hidden="true" class="w-4 h-4 mr-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M10.707 2.293a1 1 0 00-1.414 0l-7 7a1 1 0 001.414 1.414L4 10.414V17a1 1 0 001 1h2a1 1 0 001-1v-2a1 1 0 011-1h2a1 1 0 011 1v2a1 1 0 001 1h2a1 1 0 001-1v-6.586l.293.293a1 1 0 001.414-1.414l-7-"
                        />
                    </svg>
                    Home
                </router-link>
            </li>
    
            <li
                v-for="(crumb, index) in crumbs"
                :key="index"
            >
                <div class="flex items-center">
                    <svg
                        aria-hidden="true"
                        class="w-6 h-6 text-gray-400"
                        fill="currentColor"
                        viewBox="0 0 20 20"
                        xmlns="http://www.w3.org/2000/svg"
                    >
                        <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                    </svg>
                    <a
                        v-if="!isLast(index)"
                        href="#"
                        class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white"
                        @click="$emit('selected-breadcrumb', crumb)"
                    >
                        {{ crumb }}
                    </a>
                    <span
                        v-if="isLast(index)"
                        class="ml-1 text-sm font-medium text-gray-700 md:ml-2 dark:text-gray-400 dark:hover:text-white"
                    >
                        {{ crumb }}
                    </span>
                </div>
            </li>
        </ol>

        <ol>
            <li
                v-for="(routeTest, i) in routeList" :key="i"
            >
                <router-link
                    :to="{name : routeTest.name}"
                    class="inline-flex items-center text-sm font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white"
                >
                    {{ routeTest.meta.label }}
                </router-link>
            </li>
        </ol>
    </nav>
</template>

<script>
export default {
    name: 'Breadcrumb',
    props: {
        crumbs: {
            type: Array,
            required: true,
        },
    },
    data() {
        return {
            routeList: [],
        };
    },

    watch: {
        $route(to, from) {
            if (from.name !== to.name) {
                this.currentRouteList(to);
            }
        },
    },
    beforeMount() {
        this.currentRouteList(this.$route);
    },
    methods: {
        isLast(crumbIndex) {
            return this.crumbs.length === crumbIndex + 1;
        },
        currentRouteList(currentRoute) {
            this.routeList = [];
            console.log(currentRoute);
            console.log(this.$router);
            currentRoute.meta.breadcrumb.forEach((item) => {
                console.log(item);
                const routeItem = this.$router.resolve({ name: item });
                this.routeList.push(routeItem.route);
                console.log(routeItem);
            });
            this.routeList.push(currentRoute);
        },
    },
};
</script>

<style scoped>

</style>
